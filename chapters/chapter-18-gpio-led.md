...
推挽输出相当于引脚内部有两个开关：一个接 3.3V，一个接 GND。

```
输出高电平时：上面的开关接通 → 引脚 = 3.3V
               ┌── 3.3V
               ├─ 接通
               │  引脚 → 3.3V
               ├─ 断开
...
```
                    ┌── 外部上拉电阻 ── 3.3V
                    │
输出高电平时：      ├── 引脚 = 3.3V（靠外部电阻拉高）
      内部开关断开  │
                    └── GND

输出低电平时：      ├── 引脚 = 0V（内部开关接通 GND）
      内部开关接通  │
                    └── GND
```
...
当引脚设为输入模式（`Pin.IN`）时，如果什么都没接，引脚处于**悬空**状态——电压不确定，可能是高也可能低，读到的结果随机漂移。
...
**上拉电阻**（Pull-Up）就是把引脚通过一个电阻接到 3.3V——默认高电平，按下按键时拉到 GND 变成低电平：
...
按下后：GPIO25 直连 GND → 读到 0（低）
```

**下拉电阻**（Pull-Down）则相反——电阻接 GND，默认低电平，按键按下时拉到 3.3V。
